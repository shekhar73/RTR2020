...
GLuint gVao_Rect;
GLuint gVbo_Rect_Vertex;
GLuint gVbo_Rect_Color;
...
void Initialize(void)
{
	...
	glGenVertexArrays(1, &gVao_Rect);
		glBindVertexArray(gVao_Rect);
		glGenBuffers(1, &gVbo_Rect_Vertex);	//like glGenTextures() in FFP
		glBindBuffer(GL_ARRAY_BUFFER, gVbo_Rect_Vertex);	// like glBindTexture() in FFP
		glBufferData(GL_ARRAY_BUFFER, sizeof(rectVertices), rectVertices, GL_STATIC_DRAW);
		glVertexAttribPointer(SSK_ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, 0, NULL);
		glEnableVertexAttribArray(SSK_ATTRIBUTE_POSITION);
		glBindBuffer(GL_ARRAY_BUFFER, 0);

		glGenBuffers(1, &gVbo_Rect_Color);	//like glGenTextures() in FFP
		glBindBuffer(GL_ARRAY_BUFFER, gVbo_Rect_Color);	// like glBindTexture() in FFP
