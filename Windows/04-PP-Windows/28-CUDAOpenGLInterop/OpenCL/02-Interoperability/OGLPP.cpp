...
void Initialize(void)
{
	...
	glBindVertexArray(gVao);
		// For CPU
		glGenBuffers(1, &gVbo_cpu);	//like glGenTextures() in FFP
		glBindBuffer(GL_ARRAY_BUFFER, gVbo_cpu);	// like glBindTexture() in FFP
		...
		// For GPU
		glGenBuffers(1, &gVbo_gpu);	//like glGenTextures() in FFP
		glBindBuffer(GL_ARRAY_BUFFER, gVbo_gpu);	// like glBindTexture() in FFP
			glBufferData(GL_ARRAY_BUFFER, MY_ARRAY_SIZE * sizeof(float), 0, GL_DYNAMIC_DRAW);
			/*glVertexAttribPointer(SSK_ATTRIBUTE_POSITION, 4, GL_FLOAT, GL_FALSE, 0, NULL);
			glEnableVertexAttribArray(SSK_ATTRIBUTE_POSITION);*/

			// OpenGL CUDA interoperability buffer registrion
			cudaResult = cudaGraphicsGLRegisterBuffer(&cuda_graphics_resource, gVbo_gpu, cudaGraphicsMapFlagWrightDiscard);
			if(cudaResult != CUDA_SUCCESS)
			{
				DestroyWindow(ghwnd);
				UnInitialize();
...
void Display(void)
{
	...
	{
		// 1. Map Resource
		// 2. Get pointer after of CUDA
		// 3. Pass that pointer to cudaKernel and lets do the work by cudaKernel
		// 4. Now unmap the recived pointer i.e. gVbo_gpu
		// 5. Now bind the gVbo_gpu

		// 1. Map Resource
		cudaResult = cudaGraphicsMapResource(1, &cuda_graphics_resource, 0);
		if(cudaResult != CUDA_SUCCESS)
		{
			//return false;
			DestroyWindow(ghwnd);
		...
		float4 *pPos = NULL;
		size_t numBytes;
		cudaResult = cudaGraphicsResourceGetMappedPointer((void**)&pPos, &numBytes, cuda_graphics_resource);	//cuda_graphics_resource ghe and tyala pPos made MAP kar and te MAP kartana kiti bytes write kele te numBytes made de
		if(cudaResult != CUDA_SUCCESS)
		{
			DestroyWindow(ghwnd);
			UnInitialize();
		...
		// 3. Pass that pointer to cudaKernel and lets do the work by cudaKernel
		cudaResult = launchCUDAKernel(pPos, mesh_width, mesh_height, animation_time);
		if(cudaResult != CUDA_SUCCESS)
		{
			DestroyWindow(ghwnd);
			UnInitialize();
		...
		// 4. Now unmap the recived pointer i.e. gVbo_gpu
		cudaResult = cudaGraphicsUnmapResource(1, &cuda_graphics_resource, 0);
		if(cudaResult != CUDA_SUCCESS)
		{
			DestroyWindow(ghwnd);
			UnInitialize();
	...
	else
	{
		...
		glBindBuffer(GL_ARRAY_BUFFER, gVbo_cpu);
...
void UnInitialize()
{
	...
	if (gVbo_cpu)
	{
		glDeleteVertexArrays(1, &gVbo_cpu);
		gVbo_cpu = 0;
	}
