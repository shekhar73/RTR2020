...
void Initialize(void)
{
	...
	glBindVertexArray(gVao);
		...
		glBindBuffer(GL_ARRAY_BUFFER, gVbo_gpu);	// like glBindTexture() in FFP
			...
			cudaResult = cudaGraphicsGLRegisterBuffer(&cuda_graphics_resource, gVbo_gpu, cudaGraphicsMapFlagsWriteDiscard);
			if(cudaResult != cudaSuccess)
			{
				DestroyWindow(ghwnd);
				UnInitialize();
...
void Display(void)
{
	...
	if(bOnGPU == true)
	{
		...
		// 3. Pass that pointer to cudaKernel and lets do the work by cudaKernel
		launchCUDAKernel(pPos, mesh_width, mesh_height, animation_time);
