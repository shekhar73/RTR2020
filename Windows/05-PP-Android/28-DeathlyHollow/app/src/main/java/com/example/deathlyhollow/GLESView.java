package com.example.deathlyhollow;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener, OnDoubleTapListener
{
	...
	private int[] gVbo_Rectangle_Vertex = new int[1];
	private int[] gVbo_Rectangle_Color = new int[1];

	private int[] gVao_Triangle = new int[1];
	private int[] gVbo_Triangle_Vertex = new int[1];
	private int[] gVbo_Triangle_Color = new int[1];

	private int[] gVao_First_Circle = new int[1];
	private int[] gVbo_First_Circle_Vertex = new int[1];
	private int[] gVbo_First_Circle_Color = new int[1];

	private int[] gVao_Second_Circle = new int[1];
	private int[] gVbo_Second_Circle_Vertex = new int[1];
	private int[] gVbo_Second_Circle_Color = new int[1];
...
//----------------- Draw Rectangle---------------------------------------------------------
	...
	final float rectColors[] = new float[]
	{
		1.0f, 1.0f, 0.0f,
		1.0f, 1.0f, 0.0f,
	...
	GLES32.glGenVertexArrays(1, gVao_Rectangle, 0);
			GLES32.glBindVertexArray(gVao_Rectangle[0]);

			GLES32.glGenBuffers(1, gVbo_Rectangle_Vertex, 0);
			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, gVbo_Rectangle_Vertex[0]);

			byteBuffer = ByteBuffer.allocateDirect(rectVertices.length * 4);	// 4 = sizeof float
			byteBuffer.order(ByteOrder.nativeOrder());
			verticesBuffer = byteBuffer.asFloatBuffer();
			...
			GLES32.glGenBuffers(1, gVbo_Rectangle_Color, 0);
			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, gVbo_Rectangle_Color[0]);

			byteBuffer = ByteBuffer.allocateDirect(rectColors.length * 4);	// 4 = sizeof float
			byteBuffer.order(ByteOrder.nativeOrder());
			verticesBuffer = byteBuffer.asFloatBuffer();
			verticesBuffer.put(rectColors);
			verticesBuffer.position(0);
...
//----------------- Draw Triangle---------------------------------------------------------
	...
	float x2 = (float)(Math.cos(120.0f) * 0.75f);
	float y2 = -(float)(Math.sin(120.0f) * 0.75f);

	float x3 = -(float)(Math.cos(120.0f) * 0.75f);
	float y3 = -(float)(Math.sin(120.0f) * 0.75f);
	...
	GLES32.glGenVertexArrays(1, gVao_Triangle, 0);
			...
			GLES32.glGenBuffers(1, gVbo_Triangle_Vertex, 0);
			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, gVbo_Triangle_Vertex[0]);

			byteBuffer = ByteBuffer.allocateDirect(triangleVertices.length * 4);	// 4 = sizeof float
			byteBuffer.order(ByteOrder.nativeOrder());
			verticesBuffer = byteBuffer.asFloatBuffer();
			...
			GLES32.glGenBuffers(1, gVbo_Triangle_Color, 0);
			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, gVbo_Triangle_Color[0]);

			byteBuffer = ByteBuffer.allocateDirect(triangleColors.length * 4);	// 4 = sizeof float
			byteBuffer.order(ByteOrder.nativeOrder());
			verticesBuffer = byteBuffer.asFloatBuffer();
...
//----------------- Draw Circle---------------------------------------------------------
	...
	float radius = 0.75f;
	float circle1Vertices[] = new float[360 * 9];
	float circle1Colors[] = new float[360 * 9];

	float angle = 0.0f;

	for(int i = 0; i < 360 * 9; i = i + 3)
	{
		circle1Vertices[i] = radius * (float)Math.cos(angle);
		circle1Vertices[i + 1] = radius * (float)Math.sin(angle);
	...
	GLES32.glGenVertexArrays(1, gVao_First_Circle, 0);
		GLES32.glBindVertexArray(gVao_First_Circle[0]);

			GLES32.glGenBuffers(1, gVbo_First_Circle_Vertex, 0);
			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, gVbo_First_Circle_Vertex[0]);

			byteBuffer = ByteBuffer.allocateDirect(circle1Vertices.length * 4);	// 4 = sizeof float
			byteBuffer.order(ByteOrder.nativeOrder());
			verticesBuffer = byteBuffer.asFloatBuffer();
			verticesBuffer.put(circle1Vertices);
			verticesBuffer.position(0);

			GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, 
								circle1Vertices.length * 4,
								verticesBuffer,
								GLES32.GL_STATIC_DRAW);
			...
			GLES32.glGenBuffers(1, gVbo_First_Circle_Color, 0);
			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, gVbo_First_Circle_Color[0]);

			byteBuffer = ByteBuffer.allocateDirect(circle1Colors.length * 4);	// 4 = sizeof float
			byteBuffer.order(ByteOrder.nativeOrder());
			verticesBuffer = byteBuffer.asFloatBuffer();
			verticesBuffer.put(circle1Colors);
			verticesBuffer.position(0);

			GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, 
								circle1Colors.length * 4,
								verticesBuffer,
								GLES32.GL_STATIC_DRAW);
	...
	float Area = (float)Math.sqrt(Perimeter * (Perimeter - a) * (Perimeter - b) * (Perimeter - c));
	float radiusInCircle = Area / Perimeter;


	float circle2Vertices[] = new float[360 * 9];
	float circle2Colors[] = new float[360 * 9];

	angle = 0.0f;
	for (int i = 0; i < 360 * 9; i = i + 3)
	{
		circle2Vertices[i] = radiusInCircle * (float)Math.cos(angle);
		circle2Vertices[i + 1] = radiusInCircle * (float)Math.sin(angle);
	...
	GLES32.glGenVertexArrays(1, gVao_Second_Circle, 0);
		GLES32.glBindVertexArray(gVao_Second_Circle[0]);

			GLES32.glGenBuffers(1, gVbo_Second_Circle_Vertex, 0);
			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, gVbo_Second_Circle_Vertex[0]);

			byteBuffer = ByteBuffer.allocateDirect(circle2Vertices.length * 4);	// 4 = sizeof float
			byteBuffer.order(ByteOrder.nativeOrder());
			verticesBuffer = byteBuffer.asFloatBuffer();
			verticesBuffer.put(circle2Vertices);
			verticesBuffer.position(0);

			GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, 
								circle2Vertices.length * 4,
								verticesBuffer,
								GLES32.GL_STATIC_DRAW);
			...
			GLES32.glGenBuffers(1, gVbo_Second_Circle_Color, 0);
			GLES32.glBindBuffer(GLES32.GL_ARRAY_BUFFER, gVbo_Second_Circle_Color[0]);

			byteBuffer = ByteBuffer.allocateDirect(circle2Colors.length * 4);	// 4 = sizeof float
			byteBuffer.order(ByteOrder.nativeOrder());
			verticesBuffer = byteBuffer.asFloatBuffer();
			verticesBuffer.put(circle2Colors);
			verticesBuffer.position(0);

			GLES32.glBufferData(GLES32.GL_ARRAY_BUFFER, 
								circle2Colors.length * 4,
								verticesBuffer,
								GLES32.GL_STATIC_DRAW);
...
//----------------- End of Circle-----------------------------------------------
	...
	private void draw()
	{
		...
		GLES32.glBindVertexArray(gVao_First_Circle[0]);
			...
			{
				GLES32.glDrawArrays(GLES32.GL_POINTS, i, 1);
			}
		GLES32.glBindVertexArray(0);
		//--------------------------------- End of Rectangle-------------------------------------
		...
		Matrix.translateM(translateMatrix, 0, gCenterX, gCenterY, -3.0f);
		modelViewMatrix = translateMatrix;
