...
#pragma comment(lib, "DirectXTK.lib")	// For textures
...
HRESULT Initialize(void)
{
	...
	// initialize shader, input layouts, constant buffers etc.
		...
		"};"	\
		"vertex_output main(float4 pos : POSITION, float2 textures : TEXCOORD)"	\
		"{"	\
	...
	//----------------------------------------------------------------------------------------------------------
		...
		"{"	\
			"float4 myTexture2D.Sample(mySamplerState, input.texcoord);"	\
		...
		inputElementDesc[1].Format					= DXGI_FORMAT_R32G32_FLOAT;		// vertexAttribPointer cha 2nd paratmeter in OpenGL
		inputElementDesc[1].InputSlot				= 1;							// Like enum of POSITON, COLOR, NORMAL, TEXTURE0 in OpenGL
		inputElementDesc[1].AlignedByteOffset		= D3D11_APPEND_ALIGNED_ELEMENT;	// glPixelStorei for unpack alignment in OpenGL
	...
	//-----------------------------------------------------------------------------------------
		...
		gpID3D11DeviceContext->Map(
			gpID3D11Buffer_VertexBuffer_Position,		// to whom we have to map (konala mapp karaych ahe)
		...
		bufferDesc.Usage = D3D11_USAGE_DYNAMIC;	// 4th parameter of glBufferData() in OpenGL like GL_STATIC_DRAW
		bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(pyramid_texcoords); // 2nd parameter of glBufferData() in OpenGL
		bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

		hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Texture); // ya fun madla 2nd parameter ha D3D11_SUBRESOURCE_STRUCTURE ahe. It is NULL bcz we dont send static data, we are sending the data memcpy unmap way, and it is a std way of sending the data
		if (FAILED(hr))
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
		...
		gpID3D11DeviceContext->Map(
			gpID3D11Buffer_VertexBuffer_Texture,		// to whom we have to map (konala mapp karaych ahe)
			0,									// on which index we have to map, mazyakde ekach ahe, multiple nahi so 0
			D3D11_MAP_WRITE_DISCARD,			// how to map? -> discard the old/existing data and write new data
			0,									// CPU ne push karaych jar GPU busy asel tar, no need to wait so 0
			&mappedSubresource					// konashi mapp karyach ahe
		);										// antrapatha chya palikadache and alikadche data mapp karyach
		memcpy(mappedSubresource.pData, pyramid_texcoords, sizeof(pyramid_texcoords));	// vertices made aslela data pData made send karyacha ahe
		gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Texture, 0);	// pData madla data gpID3D11Buffer_VertexBuffer_Texture made janaysathi Unmap karyacha
		...
		// TEXTURES
		//----------------------------------------------------------------------
		hr = loadD3DTexture(L"Stone.bmp", &gpID3D11ShaderResourceView_Texture);	// L : literal for wid charachter
		
		if (FAILED(hr))
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: loadD3DTexture() Failed.\n");
			fclose(gpFile);
			UnInitialize();
			return(hr);
		}
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: loadD3DTexture() Succeded.\n");
			fclose(gpFile);
		}

		D3D11_SAMPLER_DESC d3d11_sampler_desc;
		ZeroMemory(&d3d11_sampler_desc, sizeof(D3D11_SAMPLER_DESC));

		d3d11_sampler_desc.Filter		= D3D11_FILTER_MIN_MAG_MIP_LINEAR;
		d3d11_sampler_desc.AddressU		= D3D11_TEXTURE_ADDRESS_WRAP;
		d3d11_sampler_desc.AddressV		= D3D11_TEXTURE_ADDRESS_WRAP;
		d3d11_sampler_desc.AddressW		= D3D11_TEXTURE_ADDRESS_WRAP;

		hr = gpID3D11Device->CreateSamplerState(&d3d11_sampler_desc, &gpID3D11SamplerState_Texture);
		if (FAILED(hr))
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: CreateSamplerState() Failed.\n");
			fclose(gpFile);
			UnInitialize();
			return(hr);
		}
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: CreateSamplerState() Succeded.\n");
			fclose(gpFile);
		}
...
HRESULT loadD3DTexture(const wchar_t* textureFileName, ID3D11ShaderResourceView** ppID3D11ShaderResourceView)
{
	...
	hr = DirectX::CreateWICTextureFromFile(
		...
		textureFileName,
		NULL,	// ID3D11ShaderResource**
		ppID3D11ShaderResourceView
	);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "loadD3DTexture() : CreateWICTextureFromFile() FAILED. Exitting now...\n");
		fclose(gpFile);
		//UnInitialize();
		return(hr);
	}
	...
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "loadD3DTexture() : CreateWICTextureFromFile() Succeded.\n");
		fclose(gpFile);
	}
...
void Display(void)
{
	...
	stride = sizeof(float) * 2;	// 2 : for texture, 3 : for color
	offset = 0;
