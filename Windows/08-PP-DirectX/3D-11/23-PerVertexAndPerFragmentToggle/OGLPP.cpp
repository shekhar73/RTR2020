...
ID3D11VertexShader* gpID3D11VertexShaderPF = NULL;
ID3D11PixelShader* gpID3D11PixelShaderPF = NULL;
...
ID3D11InputLayout* gpID3D11InputLayoutPF = NULL;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szClassName[] = TEXT("DirectX");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("Direct3D11- PerVertexPerFragmentToggle"),
	...
	case WM_KEYDOWN:
		...
		switch (wParam)
			...
			case 'l':
				...
				case 'F':
				case 'f':
					if (bLightModelToggle == false) 
					{
						bLightModelToggle = true;
					}
					break;

				case 'V':
				case 'v':
					if (bLightModelToggle == true) 
					{
						bLightModelToggle = false;
					}
					break;
...
HRESULT Initialize(void)
{
	...
	const char* vertexShaderSourceCodePV =
		...
		"{"\
			...
			"float3 light_direction = (float3)normalize(lightPosition - eyePosition);"\
			...
			"float3 specular = ls * ks * pow(max(dot(reflection_vector, viewerVector), 0.0), materialShinyness);"\
			"output.phong_ads_light = float4(ambient + diffuse + specular, 1.0f);"\
		"}"\
		"else"	\
		"{"\
			"output.phong_ads_light = float4(1.0, 1.0, 1.0, 1.0);"\
			"}"\
			"float4 positionLocal = mul(worldMatrix, pos);"\
			"positionLocal = mul(viewMatrix, positionLocal);"\
			"positionLocal = mul(projectionMatrix, positionLocal);"\
			"output.position = positionLocal;"\
			"return(output);"\
		"}";
	...
	hr = D3DCompile(vertexShaderSourceCodePV,
		lstrlenA(vertexShaderSourceCodePV) + 1,	// ASCII length
		"VS",									// Vertex shader
		NULL,									// MACRO
		...
		&pID3DBlob_VertexShaderCodePV,			// blob
	...
	hr = gpID3D11Device->CreateVertexShader(
		pID3DBlob_VertexShaderCodePV->GetBufferPointer(),
		pID3DBlob_VertexShaderCodePV->GetBufferSize(),
		NULL,
		&gpID3D11VertexShaderPV
	);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device:: CreateVertexShaderPV() Failed.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device:: CreateVertexShaderPV() Succeded.\n");
		fclose(gpFile);
	}
	...
	const char* pixelShaderSourceCodePV =
		...
		"{"	\
			"float4 color = input.phong_ads_light;"\
			"return (color);"	\
		"}";
	...
	hr = gpID3D11Device->CreatePixelShader(
		pID3DBlob_PixelShaderCodePV->GetBufferPointer(),
		pID3DBlob_PixelShaderCodePV->GetBufferSize(),
		NULL,
		&gpID3D11PixelShaderPV
	);

	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device:: CreatePixelShaderPF() Failed.\n");
		fclose(gpFile);
		return(hr);
	}
	else
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device:: CreatePixelShaderPF() Succeded.\n");
		fclose(gpFile);
	}

	// initialize create and set input layout structure
	D3D11_INPUT_ELEMENT_DESC inputElementDescPV[2];

	inputElementDescPV[0].SemanticName = "POSITION";
	inputElementDescPV[0].SemanticIndex = 0;		// if we are going to send multiple geometry in the same semanticName then by seperated by indices and this is the first geometry hence it is 0
	inputElementDescPV[0].Format = DXGI_FORMAT_R32G32B32_FLOAT; // vertexAttribPointer cha 2nd paratmeter in OpenGL
	inputElementDescPV[0].InputSlot = 0;		// Like enum of POSITON, COLOR, NORMAL, TEXTURE0 in OpenGL
	inputElementDescPV[0].AlignedByteOffset = 0;
	inputElementDescPV[0].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;	// draw vertex by using vertex data
	inputElementDescPV[0].InstanceDataStepRate = 0;		// gap between 2 instnce data

	inputElementDescPV[1].SemanticName = "NORMAL";
	inputElementDescPV[1].SemanticIndex = 0;		// if we are going to send multiple geometry in the same semanticName then by seperated by indices and this is the first geometry hence it is 0
	inputElementDescPV[1].Format = DXGI_FORMAT_R32G32B32_FLOAT; // vertexAttribPointer cha 2nd paratmeter in OpenGL
	inputElementDescPV[1].InputSlot = 1;		// Like enum of POSITON, COLOR, NORMAL, TEXTURE0 in OpenGL
	inputElementDescPV[1].AlignedByteOffset = 0;
	inputElementDescPV[1].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;	// draw vertex by using vertex data
	inputElementDescPV[1].InstanceDataStepRate = 0;		// gap between 2 instnce data

	hr = gpID3D11Device->CreateInputLayout(
		inputElementDescPV,	// not we are sending array so no & (array itself as base address)
		ARRAYSIZE(inputElementDescPV),
		pID3DBlob_VertexShaderCodePV->GetBufferPointer(),
		pID3DBlob_VertexShaderCodePV->GetBufferSize(),
		&gpID3D11InputLayoutPV
	);
	...
	const char* vertexShaderSourceCodePF =
		"cbuffer ConstantBuffer"\
		"{"\
			"float4x4 worldMatrix;"\
			"float4x4 viewMatrix;"\
	...
	ID3DBlob* pID3DBlob_VertexShaderCodePF = NULL;
	pID3DBlob_Error = NULL;

	hr = D3DCompile(
		vertexShaderSourceCodePF,
		lstrlen(vertexShaderSourceCodePF) + 1,
		"VS",
		NULL,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,
		"main",
	...
	hr = gpID3D11Device->CreateVertexShader(
		pID3DBlob_VertexShaderCodePF->GetBufferPointer(), 
		pID3DBlob_VertexShaderCodePF->GetBufferSize(), 
		NULL, 
		&gpID3D11VertexShaderPF
	);
	
	const char* pixelShaderSourceCodePF =
		"cbuffer ConstantBuffer"\
		"{"\
			"float4x4 worldMatrix;"\
			"float4x4 viewMatrix;"\
		...
		"{" \
			...
			"if(keyPressed == 1){"\
			"float3 normalized_transformed_normal = normalize(input.transformed_normals);"\
			"float3 normalized_light_direction = normalize(input.light_direction);"\
			"float3 normalized_view_vector = normalize(input.viewer_vector);"\
			"float3 ambient = (float3)(la * ka);"\
			"float3 diffuse = (float3)(ld * kd) * max(dot(normalized_light_direction, normalized_transformed_normal) , 0.0);"\
			"float3 reflection_vector = reflect(-normalized_light_direction, normalized_transformed_normal);"\
			"float3 specular = (float3)(ls * ks) * pow(max(dot(reflection_vector, normalized_view_vector), 0.0f), 50.0f);"\
	...
	ID3DBlob* pID3DBlob_PixelShaderCodePF = NULL;
	pID3DBlob_Error = NULL;
	hr = D3DCompile(pixelShaderSourceCodePF,
		lstrlen(pixelShaderSourceCodePF) + 1,
		"PS",
		NULL,
		D3D_COMPILE_STANDARD_FILE_INCLUDE,
		"main",
	...
	// Create Shader
	hr = gpID3D11Device->CreatePixelShader(
		pID3DBlob_PixelShaderCodePF->GetBufferPointer(), 
		pID3DBlob_PixelShaderCodePF->GetBufferSize(), 
		NULL, 
		&gpID3D11PixelShaderPF
	);
	
	D3D11_INPUT_ELEMENT_DESC inputElementsDescPF[2];

	inputElementsDescPF[0].SemanticName = "POSITION";
	inputElementsDescPF[0].SemanticIndex = 0;
	inputElementsDescPF[0].Format = DXGI_FORMAT_R32G32B32_FLOAT;
	inputElementsDescPF[0].InputSlot = 0;
	inputElementsDescPF[0].AlignedByteOffset = 0;
	inputElementsDescPF[0].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
	inputElementsDescPF[0].InstanceDataStepRate = 0;

	inputElementsDescPF[1].SemanticName = "NORMAL";
	inputElementsDescPF[1].SemanticIndex = 0;
	inputElementsDescPF[1].Format = DXGI_FORMAT_R32G32B32_FLOAT;
	inputElementsDescPF[1].InputSlot = 1;
	inputElementsDescPF[1].AlignedByteOffset = 0;
	inputElementsDescPF[1].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
	inputElementsDescPF[1].InstanceDataStepRate = 0;
	...
	hr = gpID3D11Device->CreateInputLayout(
		...
		pID3DBlob_VertexShaderCodePF->GetBufferPointer(),
		pID3DBlob_VertexShaderCodePF->GetBufferSize(),
		&gpID3D11InputLayoutPF
	);
	if (FAILED(hr)) {
		
		if(pID3DBlob_VertexShaderCodePF){
			pID3DBlob_VertexShaderCodePF->Release();
			pID3DBlob_VertexShaderCodePF = NULL;
		}
		return(hr);
	}
	else {
	...
	gpID3D11DeviceContext->IASetInputLayout(gpID3D11InputLayoutPF);
...
void Display(void)
{
	...
	if (bLightModelToggle == false) {
		...
		gpID3D11DeviceContext->VSSetShader(gpID3D11VertexShaderPF, 0, 0);
	}
	...
	{
		gpID3D11DeviceContext->PSSetShader(gpID3D11PixelShaderPF, 0, 0);
	}
	...
	XMMATRIX worldMatrix = XMMatrixTranslation(0.0f, 0.0f, 3.5);
	...
	if (bLight == true)
	{
		...
		constantBuffer.MaterialShinyness = materialShininess;
...
void UnInitialize()
{
	...
	if (gpID3D11InputLayoutPF)
	{
		gpID3D11InputLayoutPF->Release();
		gpID3D11InputLayoutPF = NULL;
	}
