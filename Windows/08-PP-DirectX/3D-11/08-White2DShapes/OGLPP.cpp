...
HRESULT Initialize(void)
{
	...
	//-----------------------------------------------------------------------------------------
		...
		bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(trianlgeVertices); // 2nd parameter of glBufferData() in OpenGL
		bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

		hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_triangle); // ya fun madla 2nd parameter ha D3D11_SUBRESOURCE_STRUCTURE ahe. It is NULL bcz we dont send static data, we are sending the data memcpy unmap way, and it is a std way of sending the data
		if (FAILED(hr))
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
		...
		gpID3D11DeviceContext->Map(
			gpID3D11Buffer_VertexBuffer_triangle,		// to whom we have to map (konala mapp karaych ahe)
		...
		memcpy(mappedSubresource.pData, trianlgeVertices, sizeof(trianlgeVertices));	// vertices made aslela data pData made send karyacha ahe
		gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_triangle, 0);	// pData madla data gpID3D11Buffer_VertexBuffer_triangle made janaysathi Unmap karyacha
		...
		hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_rectangle); // ya fun madla 2nd parameter ha D3D11_SUBRESOURCE_STRUCTURE ahe. It is NULL bcz we dont send static data, we are sending the data memcpy unmap way, and it is a std way of sending the data
		if (FAILED(hr))
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: CreateBuffer() Failed for Vertex Buffer.\n");
		...
		gpID3D11DeviceContext->Map(
			gpID3D11Buffer_VertexBuffer_rectangle,		// to whom we have to map (konala mapp karaych ahe)
			0,									// on which index we have to map, mazyakde ekach ahe, multiple nahi so 0
			D3D11_MAP_WRITE_DISCARD,			// how to map? -> discard the old/existing data and write new data
			0,									// CPU ne push karaych jar GPU busy asel tar, no need to wait so 0
			&mappedSubresource					// konashi mapp karyach ahe
		...
		gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_rectangle, 0);	// pData madla data gpID3D11Buffer_VertexBuffer_rectangle made janaysathi Unmap karyacha
...
void Display(void)
{
	...
	XMMATRIX worldMatrix = XMMatrixTranslation(-2.0f, 0.0f, 6.0f);
	...
	worldMatrix = XMMatrixTranslation(2.0f, 0.0f, 6.0f);
