...
ID3D11Buffer* gpID3D11Buffer_VertexBuffer_trianlge_Position = NULL;
...
HRESULT Initialize(void)
{
	...
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(triangleVertices); // 2nd parameter of glBufferData() in OpenGL
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_trianlge_Position); // ya fun madla 2nd parameter ha D3D11_SUBRESOURCE_STRUCTURE ahe. It is NULL bcz we dont send static data, we are sending the data memcpy unmap way, and it is a std way of sending the data
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	gpID3D11DeviceContext->Map(
		gpID3D11Buffer_VertexBuffer_trianlge_Position,		// to whom we have to map (konala mapp karaych ahe)
	...
	memcpy(mappedSubresource.pData, triangleVertices, sizeof(triangleVertices));	// vertices made aslela data pData made send karyacha ahe
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_trianlge_Position, 0);	// pData madla data gpID3D11Buffer_VertexBuffer_trianlge_Position made janaysathi Unmap karyacha
	...
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;	// 4th parameter of glBufferData() in OpenGL like GL_STATIC_DRAW
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(triangleColors); // 2nd parameter of glBufferData() in OpenGL
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_triangle_Color); // ya fun madla 2nd parameter ha D3D11_SUBRESOURCE_STRUCTURE ahe. It is NULL bcz we dont send static data, we are sending the data memcpy unmap way, and it is a std way of sending the data
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	gpID3D11DeviceContext->Map(
		gpID3D11Buffer_VertexBuffer_triangle_Color,		// to whom we have to map (konala mapp karaych ahe)
		0,									// on which index we have to map, mazyakde ekach ahe, multiple nahi so 0
		D3D11_MAP_WRITE_DISCARD,			// how to map? -> discard the old/existing data and write new data
		0,									// CPU ne push karaych jar GPU busy asel tar, no need to wait so 0
		&mappedSubresource					// konashi mapp karyach ahe
	);										// antrapatha chya palikadache and alikadche data mapp karyach
	memcpy(mappedSubresource.pData, triangleColors, sizeof(triangleColors));	// vertices made aslela data pData made send karyacha ahe
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_triangle_Color, 0);	// pData madla data gpID3D11Buffer_VertexBuffer_triangle_Color made janaysathi Unmap karyacha
	...
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;	// 4th parameter of glBufferData() in OpenGL like GL_STATIC_DRAW
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(rectangleVertices); // 2nd parameter of glBufferData() in OpenGL
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_rectangle_position); // ya fun madla 2nd parameter ha D3D11_SUBRESOURCE_STRUCTURE ahe. It is NULL bcz we dont send static data, we are sending the data memcpy unmap way, and it is a std way of sending the data
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device:: CreateBuffer() Failed for Vertex Buffer.\n");
	...
	gpID3D11DeviceContext->Map(
		gpID3D11Buffer_VertexBuffer_rectangle_position,		// to whom we have to map (konala mapp karaych ahe)
		0,									// on which index we have to map, mazyakde ekach ahe, multiple nahi so 0
		D3D11_MAP_WRITE_DISCARD,			// how to map? -> discard the old/existing data and write new data
		0,									// CPU ne push karaych jar GPU busy asel tar, no need to wait so 0
		&mappedSubresource					// konashi mapp karyach ahe
	);										// antrapatha chya palikadache and alikadche data mapp karyach
	memcpy(mappedSubresource.pData, rectangleVertices, sizeof(rectangleVertices));	// vertices made aslela data pData made send karyacha ahe
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_rectangle_position, 0);	// pData madla data gpID3D11Buffer_VertexBuffer made janaysathi Unmap karyacha
	...
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;	// 4th parameter of glBufferData() in OpenGL like GL_STATIC_DRAW
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(rectangleColors); // 2nd parameter of glBufferData() in OpenGL
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_rectangle_color); // ya fun madla 2nd parameter ha D3D11_SUBRESOURCE_STRUCTURE ahe. It is NULL bcz we dont send static data, we are sending the data memcpy unmap way, and it is a std way of sending the data
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device:: CreateBuffer() Failed for Vertex Buffer.\n");
	...
	gpID3D11DeviceContext->Map(
		gpID3D11Buffer_VertexBuffer_rectangle_color,		// to whom we have to map (konala mapp karaych ahe)
		0,									// on which index we have to map, mazyakde ekach ahe, multiple nahi so 0
		D3D11_MAP_WRITE_DISCARD,			// how to map? -> discard the old/existing data and write new data
		0,									// CPU ne push karaych jar GPU busy asel tar, no need to wait so 0
		&mappedSubresource					// konashi mapp karyach ahe
	);										// antrapatha chya palikadache and alikadche data mapp karyach
	memcpy(mappedSubresource.pData, rectangleColors, sizeof(rectangleColors));	// vertices made aslela data pData made send karyacha ahe
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_rectangle_color, 0);	// pData madla data gpID3D11Buffer_VertexBuffer made janaysathi Unmap karyacha
...
void Display(void)
{
	...
	gpID3D11DeviceContext->IASetVertexBuffers(0, 1, &gpID3D11Buffer_VertexBuffer_trianlge_Position, &stride, &offset);
	...
	XMMATRIX worldMatrix = XMMatrixTranslation(-2.0f, 0.0f, 5.0f);
...
void UnInitialize()
{
	...
	if (gpID3D11Buffer_VertexBuffer_trianlge_Position)
	{
		gpID3D11Buffer_VertexBuffer_trianlge_Position->Release();
		gpID3D11Buffer_VertexBuffer_trianlge_Position = NULL;
	}
