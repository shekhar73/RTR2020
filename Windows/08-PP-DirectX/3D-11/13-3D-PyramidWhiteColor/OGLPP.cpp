...
// color rectanlge
	...
	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Color); // ya fun madla 2nd parameter ha D3D11_SUBRESOURCE_STRUCTURE ahe. It is NULL bcz we dont send static data, we are sending the data memcpy unmap way, and it is a std way of sending the data
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	gpID3D11DeviceContext->Map(
		gpID3D11Buffer_VertexBuffer_Color,		// to whom we have to map (konala mapp karaych ahe)
		0,									// on which index we have to map, mazyakde ekach ahe, multiple nahi so 0
		D3D11_MAP_WRITE_DISCARD,			// how to map? -> discard the old/existing data and write new data
		0,									// CPU ne push karaych jar GPU busy asel tar, no need to wait so 0
		&mappedSubresource					// konashi mapp karyach ahe
	...
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Color, 0);	// pData madla data gpID3D11Buffer_VertexBuffer_Color made janaysathi Unmap karyacha
...
void Display(void)
{
	...
	XMMATRIX translationMatrix = XMMatrixTranslation(0.0f, 0.0f, 7.0f);
	...
	XMMATRIX worldMatrix = rotationMatrix_X * rotationMatrix_Y * rotationMatrix_Z * translationMatrix;
