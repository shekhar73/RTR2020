	...
	const char* vertexShaderSourceCode =
		...
		"{"	\
			...
			"{"	\
				...
				"output.transformed_normals = (float3)normalize(mul((float3x3)worldMatrix, (float3)normal));"	\
				"float3 light_direction = normalize(light_position - eye_position);"	\
				"float3 ambient_light = la * ka;"	\
				"float3 diffuse_light = ld * kd * max(dot(light_direction, transformed_normals), 0.0);"	\
				"float3 reflection_vector = reflect(-light_direction, transformed_normals);"	\
				"float3 viewer_vector = normalize(-eye_position.xyz);"	\
				"float3 specular = ls * ks * pow(max(dot(reflection_vector, viewer_vector), 0.0), material_shininess);"	\
				"output.phong_ads_light = ambient_light + diffuse_light + specular;"	\
			"}"	\
			"else"	\
			"{"	\
...
void Display(void)
{
	...
	XMMATRIX translationMatrix = XMMatrixTranslation(0.0f, 0.0f, 2.5f);
	XMMATRIX viewMatrix	 = XMMatrixIdentity();
	XMMATRIX rotationMatrix_Y = XMMatrixRotationY(angle_cube);

	XMMATRIX worldMatrix = rotationMatrix_Y * translationMatrix;
