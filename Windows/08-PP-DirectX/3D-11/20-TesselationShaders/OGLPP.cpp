...
ID3D11DomainShader* gpID3D11DomainShader					= NULL;		// Tesselation evalution shader in OpenGL
...
{
	XMVECTOR hull_constant_functions_params;
};
...
// XMVECTOR is an array of 4 ... listen recording it is imp note
...
HRESULT Initialize(void)
{
	...
	// initialize shader, input layouts, constant buffers etc.
		...
		const char* hullShaderSourceCode =
			...
			"float number_of_line_strip = hull_constant_function_params[0];"	\
			"float number_of_line_segment = hull_constant_function_params[1];"	\
			"output.edges[0] = number_of_line_strip;"	\
			"output.edges[1] = number_of_line_segment;"	\
			"return(output);"\
			"}"	\
			"struct hull_output"	\
			"{"	\
		...
		hr = D3DCompile(hullShaderSourceCode,
			lstrlen(hullShaderSourceCode) + 1,
			"HS",
			NULL,
			D3D_COMPILE_STANDARD_FILE_INCLUDE,
			"main",
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: CreateHullShader() Succeded.\n");
			fclose(gpFile);
		}
		...
		const char* domainShaderSourceCode =
			...
			"float3 finalP = p0 * (1.0 - tessCoord.x) * (1.0 - tessCoord.x) * (1.0 - tessCoord.x) + p1 * 3.0f * tessCoord.x * (1.0f - tessCoord.x) * (1.0f - tessCoord.x) + p2 * 3.0f * tessCoord.x * tessCoord.x * (1.0f - tessCoord.x) + p3 * tessCoord.x * tessCoord.x * tessCoord.x;"
			"float4 position = float4(finalP, 1.0);"\
		...
		hr = D3DCompile(domainShaderSourceCode,
			lstrlen(domainShaderSourceCode) + 1,
			"DS",
			NULL,
			D3D_COMPILE_STANDARD_FILE_INCLUDE,
			"main",
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: CreateDomainShader() Succeded.\n");
			fclose(gpFile);
		}
	...
	//-----------------------------------------------------------------------------------------
		...
		bufferDesc_ConstantBuffer_HullShader.CPUAccessFlags = 0;

		hr = gpID3D11Device->CreateBuffer(&bufferDesc_ConstantBuffer_HullShader, nullptr, &gpID3D11Buffer_ConstantBuffer_HullShader);
		if (FAILED(hr))
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: CreateBuffer() Failed for bufferDesc_ConstantBuffer_HullShader Buffer.\n");
			fclose(gpFile);
			return(hr);
		}
		else
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: CreateBuffer() Succeded for bufferDesc_ConstantBuffer_HullShader Buffer.\n");
			fclose(gpFile);
		}
		...
		gpID3D11DeviceContext->HSSetConstantBuffers(
			0, 	// 0th index
			1, // how many buffers
			&gpID3D11Buffer_ConstantBuffer_HullShader	// which buffer
		);
		...
		bufferDesc_ConstantBuffer_DomainShader.CPUAccessFlags = 0;

		hr = gpID3D11Device->CreateBuffer(&bufferDesc_ConstantBuffer_DomainShader, nullptr, &gpID3D11Buffer_ConstantBuffer_DomainShader);
		if (FAILED(hr))
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: CreateBuffer() Failed for DomainShader Buffer.\n");
			fclose(gpFile);
			return(hr);
		}
		else
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: CreateBuffer() Succeded for DomainShader Buffer.\n");
			fclose(gpFile);
		}
		...
		gpID3D11DeviceContext->DSSetConstantBuffers(
			0, 	// 0th index
			1, // how many buffers
			&gpID3D11Buffer_ConstantBuffer_DomainShader	// which buffer
		);
		...
		bufferDesc_ConstantBuffer_PixelShader.CPUAccessFlags = 0;

		hr = gpID3D11Device->CreateBuffer(&bufferDesc_ConstantBuffer_PixelShader, nullptr, &gpID3D11Buffer_ConstantBuffer_PixelShader);
		if (FAILED(hr))
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: CreateBuffer() Failed for bufferDesc_ConstantBuffer_PixelShader Buffer.\n");
			fclose(gpFile);
			return(hr);
		}
		else
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: CreateBuffer() Succeded for bufferDesc_ConstantBuffer_PixelShader Buffer.\n");
			fclose(gpFile);
		}
		...
		gpID3D11DeviceContext->PSSetConstantBuffers(
			0, 	// 0th index
			1, // how many buffers
			&gpID3D11Buffer_ConstantBuffer_PixelShader	// which buffer
		);
...
void Display(void)
{
	...
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer_DomainShader, 0, NULL, &constantBuffer_DomainShader, 0, 0); //  like bufferData 

	
	CBUFFER_HullShader constantBuffer_HullShader;
	ZeroMemory((void*)&constantBuffer_HullShader, sizeof(CBUFFER_HullShader));
	constantBuffer_HullShader.hull_constant_functions_params = XMVectorSet(1.0f, (FLOAT)uiNumberOfLineSegments, 0.0f, 1.0f);
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer_HullShader, 0, NULL, &constantBuffer_HullShader, 0, 0); //  like bufferData 
	...
	gpID3D11DeviceContext->UpdateSubresource(gpID3D11Buffer_ConstantBuffer_PixelShader, 0, NULL, &constantBuffer_PixelShader, 0, 0); //  like bufferData 
