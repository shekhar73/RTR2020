...
HRESULT Initialize(void)
{
	...
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;	// 4th parameter of glBufferData() in OpenGL like GL_STATIC_DRAW
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(pyramid_colors); // 2nd parameter of glBufferData() in OpenGL
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Pyramid_Color); // ya fun madla 2nd parameter ha D3D11_SUBRESOURCE_STRUCTURE ahe. It is NULL bcz we dont send static data, we are sending the data memcpy unmap way, and it is a std way of sending the data
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
	...
	gpID3D11DeviceContext->Map(
		gpID3D11Buffer_VertexBuffer_Pyramid_Color,		// to whom we have to map (konala mapp karaych ahe)
		0,									// on which index we have to map, mazyakde ekach ahe, multiple nahi so 0
		D3D11_MAP_WRITE_DISCARD,			// how to map? -> discard the old/existing data and write new data
		0,									// CPU ne push karaych jar GPU busy asel tar, no need to wait so 0
		&mappedSubresource					// konashi mapp karyach ahe
	);										// antrapatha chya palikadache and alikadche data mapp karyach
	memcpy(mappedSubresource.pData, pyramid_colors, sizeof(pyramid_colors));	// vertices made aslela data pData made send karyacha ahe
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Pyramid_Color, 0);	// pData madla data gpID3D11Buffer_VertexBuffer_Color made janaysathi Unmap karyacha
	...
	bufferDesc.Usage			= D3D11_USAGE_DYNAMIC;	// 4th parameter of glBufferData() in OpenGL like GL_STATIC_DRAW
	bufferDesc.ByteWidth		= sizeof(float) * ARRAYSIZE(cube_vertices); // 2nd parameter of glBufferData() in OpenGL
	bufferDesc.BindFlags		= D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags	= D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Cube_Position); // ya fun madla 2nd parameter ha D3D11_SUBRESOURCE_STRUCTURE ahe. It is NULL bcz we dont send static data, we are sending the data memcpy unmap way, and it is a std way of sending the data
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device:: CreateBuffer() Failed for Vertex Buffer.\n");
	...
	gpID3D11DeviceContext->Map(
		gpID3D11Buffer_VertexBuffer_Cube_Position,		// to whom we have to map (konala mapp karaych ahe)
		0,									// on which index we have to map, mazyakde ekach ahe, multiple nahi so 0
		D3D11_MAP_WRITE_DISCARD,			// how to map? -> discard the old/existing data and write new data
		0,									// CPU ne push karaych jar GPU busy asel tar, no need to wait so 0
		&mappedSubresource					// konashi mapp karyach ahe
	);										// antrapatha chya palikadache and alikadche data mapp karyach
	memcpy(mappedSubresource.pData, cube_vertices, sizeof(cube_vertices));	// vertices made aslela data pData made send karyacha ahe
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Cube_Position, 0);	// pData madla data gpID3D11Buffer_VertexBuffer_Position made janaysathi Unmap karyacha
	...
	bufferDesc.Usage = D3D11_USAGE_DYNAMIC;	// 4th parameter of glBufferData() in OpenGL like GL_STATIC_DRAW
	bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(cube_colors); // 2nd parameter of glBufferData() in OpenGL
	bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

	hr = gpID3D11Device->CreateBuffer(&bufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_Cube_Color); // ya fun madla 2nd parameter ha D3D11_SUBRESOURCE_STRUCTURE ahe. It is NULL bcz we dont send static data, we are sending the data memcpy unmap way, and it is a std way of sending the data
	if (FAILED(hr))
	{
		fopen_s(&gpFile, gszLogFileName, "a+");
		fprintf_s(gpFile, "ID3D11Device:: CreateBuffer() Failed for color Buffer.\n");
	...
	gpID3D11DeviceContext->Map(
		gpID3D11Buffer_VertexBuffer_Cube_Color,		// to whom we have to map (konala mapp karaych ahe)
		0,									// on which index we have to map, mazyakde ekach ahe, multiple nahi so 0
		D3D11_MAP_WRITE_DISCARD,			// how to map? -> discard the old/existing data and write new data
		0,									// CPU ne push karaych jar GPU busy asel tar, no need to wait so 0
		&mappedSubresource					// konashi mapp karyach ahe
	);										// antrapatha chya palikadache and alikadche data mapp karyach
	memcpy(mappedSubresource.pData, cube_colors, sizeof(cube_colors));	// vertices made aslela data pData made send karyacha ahe
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_Cube_Color, 0);	// pData madla data gpID3D11Buffer_VertexBuffer_Color made janaysathi Unmap karyacha
...
void Display(void)
{
	...
	XMMATRIX translationMatrix = XMMatrixTranslation(-3.0f, 0.0f, 7.0f);
	XMMATRIX rotationMatrix = XMMatrixRotationY(angle_pyramid);
	...
	// translation is concerned with world matirx transformation
	translationMatrix = XMMatrixTranslation(3.0f, 0.0f, 8.0f);
	XMMATRIX rotationMatrix_X = XMMatrixRotationX(angle_cube);
	XMMATRIX rotationMatrix_Y = XMMatrixRotationY(angle_cube);
	XMMATRIX rotationMatrix_Z = XMMatrixRotationZ(angle_cube);
	worldMatrix = rotationMatrix_X * rotationMatrix_Y * rotationMatrix_Z * translationMatrix;
	...
	angle_pyramid = angle_pyramid + 0.0001f;
	if (angle_pyramid >= 360.0f)
	{
		angle_pyramid = 0.0f;
	}

	angle_cube = angle_cube + 0.0001f;
