	...
	const char* vertexShaderSourceCode =
		...
		"{"	\
			...
			"{"	\
				...
				"float3 source = normalize(light_position - eye_position);"	\
...
void Display(void)
{
	...
	XMMATRIX wvpMatrix = worldMatrix * viewMatrix;

	// load the data into the constant buffer
	CBUFFER constantBuffer;
	constantBuffer.WorldViewMatrix = wvpMatrix;
	constantBuffer.ProjectionMatrix = gPerspectiveProjectionMatrix;

	if (bLight == true)
	{
		constantBuffer.KeyPressed = 1;
		constantBuffer.Ld = XMVectorSet(1.0f, 1.0f, 1.0f, 1.0f);	// white color light
