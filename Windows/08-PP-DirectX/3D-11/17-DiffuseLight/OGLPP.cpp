	...
	const char* vertexShaderSourceCode =
		...
		"{"	\
			...
			"{"	\
				...
				"float3 transformed_normals = (float3)normalize(mul((float3x3)worldViewMatrix, (float3)normal));"	\
				"float3 light_direction = normalize(light_position - eye_coordinates);"	\
				"output.diffuse_light = ld * kd * max(dot(light_direction, transformed_normals), 0.0);"	\
			"}"	\
			"else"	\
			"{"	\
...
void Display(void)
{
	...
	if (bLight == true)
	{
		...
		constantBuffer.Ld = XMVectorSet(1.0f, 0.0f, 1.0f, 1.0f);
