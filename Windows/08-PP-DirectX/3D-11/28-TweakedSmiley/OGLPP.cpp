...
HRESULT Initialize(void)
{
	...
	//-----------------------------------------------------------------------------------------
		...
		bufferDesc_VertexBuffer_Position.ByteWidth = sizeof(float) * _ARRAYSIZE(vertices);
		bufferDesc_VertexBuffer_Position.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		bufferDesc_VertexBuffer_Position.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
		...
		D3D11_MAPPED_SUBRESOURCE mappedSubresource_position;
		ZeroMemory(&mappedSubresource_position, sizeof(D3D11_MAPPED_SUBRESOURCE));
		gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Position, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource_position);
		memcpy(mappedSubresource_position.pData, vertices, sizeof(vertices));
		...
		bufferDesc_VertexBuffer_Texture.ByteWidth = sizeof(float) * 12;
		bufferDesc_VertexBuffer_Texture.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		bufferDesc_VertexBuffer_Texture.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;

		hr = gpID3D11Device->CreateBuffer(&bufferDesc_VertexBuffer_Texture, NULL, &gpID3D11Buffer_VertexBuffer_Texture); // ya fun madla 2nd parameter ha D3D11_SUBRESOURCE_STRUCTURE ahe. It is NULL bcz we dont send static data, we are sending the data memcpy unmap way, and it is a std way of sending the data
		if (FAILED(hr))
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: CreateRasterizerState() Succeded for Constant Buffer.\n");
			fclose(gpFile);
		}
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: LoadD3DTexture() Failed for Constant Buffer.\n");
			fclose(gpFile);
			UnInitialize();
			return(hr);
		}
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: LoadD3DTexture() Succeded for Constant Buffer.\n");
			fclose(gpFile);
		}
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: XMMatrixIdentity() Failed for Constant Buffer.\n");
			fclose(gpFile);
			UnInitialize();
			return(hr);
		}
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: XMMatrixIdentity() Succeded for Constant Buffer.\n");
			fclose(gpFile);
		}
...
HRESULT LoadD3DTexture(const wchar_t *textureFileName, ID3D11ShaderResourceView **ppID3D11ShaderResourceView) 
{
	...
	{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: CreateWICTextureFromFile() Failed for Constant Buffer.\n");
			fclose(gpFile);
			UnInitialize();
			return(hr);
		}
		...
		{
			fopen_s(&gpFile, gszLogFileName, "a+");
			fprintf_s(gpFile, "ID3D11Device:: CreateWICTextureFromFile() Succeded for Constant Buffer.\n");
			fclose(gpFile);
		}
...
void Display(void)
{
	...
	float texcoords[12];
	ZeroMemory(&texcoords, sizeof(float) * _ARRAYSIZE(texcoords));
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_Texture, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedSubresource_texture);
	...
	constantBuffer.KeyPress = keyPress;
