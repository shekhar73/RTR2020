...
HRESULT Initialize(void)
{
	...
	//-----------------------------------------------------------------------------------------
		...
		bufferDesc.Usage = D3D11_USAGE_DYNAMIC;	// 4th parameter of glBufferData() in OpenGL like GL_STATIC_DRAW
		bufferDesc.ByteWidth = sizeof(float) * ARRAYSIZE(colors); // 2nd parameter of glBufferData() in OpenGL
		bufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;
		bufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
		...
		memcpy(mappedSubresource.pData, colors, sizeof(colors));	// vertices made aslela data pData made send karyacha ahe
...
void Display(void)
{
	...
	XMMATRIX rotationMatrix = XMMatrixRotationY(angle_triangle);
	...
	angle_triangle = angle_triangle + 0.001f;
